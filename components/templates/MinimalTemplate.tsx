      ...
      <div className="space-y-10">
        {hasPersonalInfo(bioData) && (
          <MinimalSection title={t('template.personalInfo')}>
            <div className="grid grid-cols-2 gap-x-12 gap-y-4">
              ...
              {personalInfo.complexion && <Row label={t('template.complexion')} value={tv(personalInfo.complexion, 'complexion')} />}
              {personalInfo.bloodGroup && <Row label={t('template.bloodGroup')} value={tv(personalInfo.bloodGroup, 'bloodGroup')} />}
              {personalInfo.maritalStatus && <Row label={t('template.maritalStatus')} value={tv(personalInfo.maritalStatus, 'maritalStatus')} />}
        ...
        {hasReligiousInfo(bioData) && (
          <MinimalSection title={t('template.religiousInfo')}>
            <div className="grid grid-cols-2 gap-x-12 gap-y-4">
              {religiousInfo.prayerPractice && religiousInfo.prayerPractice !== 'None' && <Row label={t('template.prayerPractice')} value={tv(religiousInfo.prayerPractice, 'prayer')} />}
              {religiousInfo.quranRecitation && religiousInfo.quranRecitation !== 'None' && <Row label={t('template.quranRecitation')} value={tv(religiousInfo.quranRecitation, 'quran')} />}
              {religiousInfo.islamicKnowledge && religiousInfo.islamicKnowledge !== 'None' && <Row label={t('template.islamicKnowledge')} value={tv(religiousInfo.islamicKnowledge, 'islamicKnowledge')} />}
              {religiousInfo.hijabOrBeard && religiousInfo.hijabOrBeard !== 'None' && <Row label={t('template.hijabBeard')} value={tv(religiousInfo.hijabOrBeard, 'hijabBeard')} />}
              {religiousInfo.sect && religiousInfo.sect !== 'None' && <Row label={t('template.sect')} value={tv(religiousInfo.sect, 'sect')} />}
        ...
        {hasProfessionalInfo(bioData) && (
          <MinimalSection title={t('template.professionalInfo')}>
            <div className="grid grid-cols-2 gap-x-12 gap-y-4">
              ...
              {professionalInfo.incomeRange && <Row label={t('template.annualIncome')} value={tv(professionalInfo.incomeRange, 'income')} />}
        ...
        {hasFamilyInfo(bioData) && (
          <MinimalSection title={t('template.familyBackground')}>
            <div className="grid grid-cols-2 gap-x-12 gap-y-4">
              ...
              {familyInfo.familyType && <Row label={t('template.familyType')} value={tv(familyInfo.familyType, 'familyType')} />}
              {familyInfo.economicStatus && <Row label={t('template.economicStatus')} value={tv(familyInfo.economicStatus, 'economicStatus')} />}
...
const MinimalSection: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <section>
    <h2 className="mb-5 text-xs font-medium uppercase tracking-widest text-slate-600">{title}</h2>
