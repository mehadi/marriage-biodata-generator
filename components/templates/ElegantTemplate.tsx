          ...
          <div className="space-y-6">
            {hasPersonalInfo(bioData) && (
              <ElegantSection title={t('template.personalDetails')}>
                ...
                {personalInfo.complexion && <ElegantField label={t('template.complexion')} value={tv(personalInfo.complexion, 'complexion')} />}
                {personalInfo.bloodGroup && <ElegantField label={t('template.bloodGroup')} value={tv(personalInfo.bloodGroup, 'bloodGroup')} />}
                {personalInfo.maritalStatus && <ElegantField label={t('template.maritalStatus')} value={tv(personalInfo.maritalStatus, 'maritalStatus')} />}
                ...
                {getCustomFieldsForSection(bioData, 'personal-info').map((f) => (
                  <ElegantField key={f.key} label={f.key} value={f.value} />
                ))}
              </ElegantSection>
            )}
            ...
            {hasReligiousInfo(bioData) && (
              <ElegantSection title={t('template.religiousProfile')}>
                {religiousInfo.prayerPractice && religiousInfo.prayerPractice !== 'None' && <ElegantField label={t('template.prayerPractice')} value={tv(religiousInfo.prayerPractice, 'prayer')} />}
                {religiousInfo.quranRecitation && religiousInfo.quranRecitation !== 'None' && <ElegantField label={t('template.quranRecitation')} value={tv(religiousInfo.quranRecitation, 'quran')} />}
                {religiousInfo.islamicKnowledge && religiousInfo.islamicKnowledge !== 'None' && <ElegantField label={t('template.islamicKnowledge')} value={tv(religiousInfo.islamicKnowledge, 'islamicKnowledge')} />}
                {religiousInfo.hijabOrBeard && religiousInfo.hijabOrBeard !== 'None' && <ElegantField label={t('template.hijabBeard')} value={tv(religiousInfo.hijabOrBeard, 'hijabBeard')} />}
                {religiousInfo.sect && religiousInfo.sect !== 'None' && <ElegantField label={t('template.sect')} value={tv(religiousInfo.sect, 'sect')} />}
          ...
          <div className="space-y-6">
            {hasProfessionalInfo(bioData) && (
              <ElegantSection title={t('template.professionalInfo')}>
                ...
                {professionalInfo.incomeRange && <ElegantField label={t('template.annualIncome')} value={tv(professionalInfo.incomeRange, 'income')} />}
            ...
            {hasFamilyInfo(bioData) && (
              <ElegantSection title={t('template.familyBackground')}>
                ...
                {familyInfo.familyType && <ElegantField label={t('template.familyType')} value={tv(familyInfo.familyType, 'familyType')} />}
                {familyInfo.economicStatus && <ElegantField label={t('template.economicStatus')} value={tv(familyInfo.economicStatus, 'economicStatus')} />}
          ...
          <div className="mt-8 rounded-lg bg-gradient-to-r from-blue-600 to-blue-800 p-6 text-white">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {getCustomFieldsForSection(bioData, 'contact').map((f) => (
                <ContactField key={f.key} label={f.key} value={f.value} />
              ))}
            </div>
