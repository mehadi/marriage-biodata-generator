        ...
        <div className="space-y-6">
          {hasPersonalInfo(bioData) && (
            <GradSection title={t('template.personalInfo')}>
              <div className="grid grid-cols-2 gap-3">
                ...
                {personalInfo.complexion && <GradRow label={t('template.complexion')} value={tv(personalInfo.complexion, 'complexion')} />}
                {personalInfo.bloodGroup && <GradRow label={t('template.bloodGroup')} value={tv(personalInfo.bloodGroup, 'bloodGroup')} />}
                {personalInfo.maritalStatus && <GradRow label={t('template.maritalStatus')} value={tv(personalInfo.maritalStatus, 'maritalStatus')} />}
                ...
                {getCustomFieldsForSection(bioData, 'personal-info').map((f) => (
                  <GradRow key={f.key} label={f.key} value={f.value} />
                ))}
              </div>
            </GradSection>
          )}
          ...
          {hasReligiousInfo(bioData) && (
            <GradSection title={t('template.religiousInfo')}>
              <div className="grid grid-cols-2 gap-3">
                {religiousInfo.prayerPractice && religiousInfo.prayerPractice !== 'None' && <GradRow label={t('template.prayerPractice')} value={tv(religiousInfo.prayerPractice, 'prayer')} />}
                {religiousInfo.quranRecitation && religiousInfo.quranRecitation !== 'None' && <GradRow label={t('template.quranRecitation')} value={tv(religiousInfo.quranRecitation, 'quran')} />}
                {religiousInfo.islamicKnowledge && religiousInfo.islamicKnowledge !== 'None' && <GradRow label={t('template.islamicKnowledge')} value={tv(religiousInfo.islamicKnowledge, 'islamicKnowledge')} />}
                {religiousInfo.hijabOrBeard && religiousInfo.hijabOrBeard !== 'None' && <GradRow label={t('template.hijabBeard')} value={tv(religiousInfo.hijabOrBeard, 'hijabBeard')} />}
                {religiousInfo.sect && religiousInfo.sect !== 'None' && <GradRow label={t('template.sect')} value={tv(religiousInfo.sect, 'sect')} />}
          ...
          {hasProfessionalInfo(bioData) && (
            <GradSection title={t('template.professionalInfo')}>
              <div className="grid grid-cols-2 gap-3">
                ...
                {professionalInfo.incomeRange && <GradRow label={t('template.annualIncome')} value={tv(professionalInfo.incomeRange, 'income')} />}
          ...
          {hasFamilyInfo(bioData) && (
            <GradSection title={t('template.familyBackground')}>
              <div className="grid grid-cols-2 gap-3">
                ...
                {familyInfo.familyType && <GradRow label={t('template.familyType')} value={tv(familyInfo.familyType, 'familyType')} />}
                {familyInfo.economicStatus && <GradRow label={t('template.economicStatus')} value={tv(familyInfo.economicStatus, 'economicStatus')} />}
            ...
            <div className="rounded-xl bg-gradient-to-r from-teal-500 to-cyan-600 p-6 text-white">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {getCustomFieldsForSection(bioData, 'contact').map((f) => (
                  <GradRowLight key={f.key} label={f.key} value={f.value} />
                ))}
              </div>
