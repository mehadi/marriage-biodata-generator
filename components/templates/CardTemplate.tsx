      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        {hasPersonalInfo(bioData) && (
          <Card title={t('template.personalInfo')}>
            ...
            {personalInfo.complexion && <CardRow label={t('template.complexion')} value={tv(personalInfo.complexion, 'complexion')} />}
            {personalInfo.bloodGroup && <CardRow label={t('template.bloodGroup')} value={tv(personalInfo.bloodGroup, 'bloodGroup')} />}
            {personalInfo.maritalStatus && <CardRow label={t('template.maritalStatus')} value={tv(personalInfo.maritalStatus, 'maritalStatus')} />}
            ...
            {getCustomFieldsForSection(bioData, 'personal-info').map((f) => (
              <CardRow key={f.key} label={f.key} value={f.value} />
            ))}
          </Card>
        )}
        ...
        {hasReligiousInfo(bioData) && (
          <Card title={t('template.religiousInfo')}>
            {religiousInfo.prayerPractice && religiousInfo.prayerPractice !== 'None' && <CardRow label={t('template.prayerPractice')} value={tv(religiousInfo.prayerPractice, 'prayer')} />}
            {religiousInfo.quranRecitation && religiousInfo.quranRecitation !== 'None' && <CardRow label={t('template.quranRecitation')} value={tv(religiousInfo.quranRecitation, 'quran')} />}
            {religiousInfo.islamicKnowledge && religiousInfo.islamicKnowledge !== 'None' && <CardRow label={t('template.islamicKnowledge')} value={tv(religiousInfo.islamicKnowledge, 'islamicKnowledge')} />}
            {religiousInfo.hijabOrBeard && religiousInfo.hijabOrBeard !== 'None' && <CardRow label={t('template.hijabBeard')} value={tv(religiousInfo.hijabOrBeard, 'hijabBeard')} />}
            {religiousInfo.sect && religiousInfo.sect !== 'None' && <CardRow label={t('template.sect')} value={tv(religiousInfo.sect, 'sect')} />}
        ...
        {hasProfessionalInfo(bioData) && (
          <Card title={t('template.professionalInfo')}>
            ...
            {professionalInfo.incomeRange && <CardRow label={t('template.annualIncome')} value={tv(professionalInfo.incomeRange, 'income')} />}
        ...
        {hasFamilyInfo(bioData) && (
          <Card title={t('template.familyBackground')}>
            ...
            {familyInfo.familyType && <CardRow label={t('template.familyType')} value={tv(familyInfo.familyType, 'familyType')} />}
            {familyInfo.economicStatus && <CardRow label={t('template.economicStatus')} value={tv(familyInfo.economicStatus, 'economicStatus')} />}
