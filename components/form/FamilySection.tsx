...
import { DynamicKeyValueField } from '@/components/form/DynamicKeyValueField';
  ...
  return (
    <Card id="family">
      ...
      <CardContent>
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-medium text-slate-900 dark:text-slate-100">{t('form.family.siblings')}</h4>
              <Button
                type="button"
                variant="outline"
                size="sm"
            ...
            {fields.length === 0 && (
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('form.family.noSiblings')}</p>
            )}

            {fields.map((field, index) => (
              <div key={field.id} className="rounded-lg border border-slate-200 p-4 dark:border-slate-700">
                <div className="mb-3 flex items-center justify-between">
                  <h5 className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('form.family.siblingNumber', { n: String(index + 1) })}</h5>
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
        ...
        <DynamicKeyValueField sectionId="family" control={control} />
      </CardContent>
    </Card>
  );
};
