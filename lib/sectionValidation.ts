...
function hasCustomFieldsForSection(data: Partial<BioData>, sectionId: CustomFieldsSectionId): boolean {
  const entries = data.customFields?.[sectionId] ?? [];
  return entries.some((e) => e?.key?.trim() && e?.value?.trim());
}

/**
 * Check if personal info section has sufficient data
 * Shows section if ANY field is filled (including custom fields)
 */
export function hasPersonalInfo(data: Partial<BioData>): boolean {
  if (hasCustomFieldsForSection(data, 'personal-info')) return true;
...
export function hasReligiousInfo(data: Partial<BioData>): boolean {
  if (hasCustomFieldsForSection(data, 'religious-info')) return true;
...
export function hasEducation(data: Partial<BioData>): boolean {
  if (hasCustomFieldsForSection(data, 'education')) return true;
...
export function hasProfessionalInfo(data: Partial<BioData>): boolean {
  if (hasCustomFieldsForSection(data, 'professional')) return true;
...
export function hasFamilyInfo(data: Partial<BioData>): boolean {
  if (hasCustomFieldsForSection(data, 'family')) return true;
...
export function hasContactInfo(data: Partial<BioData>): boolean {
  if (hasCustomFieldsForSection(data, 'contact')) return true;
...
export function hasExpectations(data: Partial<BioData>): boolean {
  if (hasCustomFieldsForSection(data, 'expectations')) return true;
